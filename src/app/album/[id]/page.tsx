...
type AlbumWithTracks = DeezerAlbum & {
...
export default function AlbumPage() {
  ...
  const [data, setData] = useState<AlbumWithTracks | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const { setQueue } = usePlayer();

  const loadAlbum = useEffectEvent((albumId: string) => {
    setLoading(true);
    setError(null);
    fetch(`/api/deezer/album/${albumId}`)
      .then((res) => {
        if (!res.ok) throw new Error("Álbum não encontrado");
        return res.json() as Promise<AlbumWithTracks>;
      })
      .then(setData)
      .catch((e: unknown) =>
  ...
  if (loading) {
    return (
      <div className="flex min-h-[40vh] items-center justify-center">
        <p className="text-sm text-zinc-400">Carregando álbum…</p>
      </div>
    );
  }
  ...
  if (error || !data) {
    return (
      <div className="rounded-2xl border border-red-500/20 bg-red-500/10 p-6 text-center">
        <p className="text-red-200">{error ?? "Álbum não encontrado."}</p>
        <Link
          href="/"
  ...
  const cover = data.cover_big ?? data.cover_medium;
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-end">
        ...
        <div className="min-w-0 flex-1">
          ...
          <Link
            ...
            className="mt-1 block text-sm text-zinc-400 hover:text-violet-400"
          >
            {data.artist?.name}
          </Link>
